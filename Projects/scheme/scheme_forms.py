...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_value = do_lambda_form(Pair(formals, body), env)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        values = Pair(eval_all(binding.rest, env), values)
...
def do_enumerate(expressions, env):
    print('DEBUG: START do_enumerate ', expressions.first)
    expr = scheme_eval(expressions.first, env)
    # Flip expr for later calculation
    reverse_expr = nil
    index = -1
    while expr != nil:
        value = expr.first
        reverse_expr = Pair(value, reverse_expr)
        expr = expr.rest
        index += 1
    
    res = nil
    while reverse_expr != nil:
        value = reverse_expr.first
        res = Pair(Pair(index, Pair(value, nil)), res)
        reverse_expr = reverse_expr.rest
        index -= 1
    return res
