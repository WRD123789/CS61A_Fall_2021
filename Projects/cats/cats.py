...
def accuracy(typed, reference):
    ...
    if typed_words == [] and reference_words == []:
        ...
        res = 0
        n, m = len(typed_words), len(reference_words)
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    min_word = min(valid_words, \
        key = lambda word: diff_function(word, typed_word, limit))
    
    if diff_function(min_word, typed_word, limit) <= limit:
        return min_word
    else:
        return typed_word
    # END PROBLEM 5
...
def feline_flips(start, goal, limit):
    ...
    if not start:
        ...
        is_equal = start[0] != goal[0]
        return feline_flips(start[1:], goal[1:], limit - is_equal) \
            + is_equal
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit == 0:
        return start != goal
    # Dynamic programming-editing distance
    if not start:
        return len(goal)
    elif not goal:
        return len(start)
    else:
        return min(minimum_mewtations(start[:-1], goal, limit - 1) + 1,
        minimum_mewtations(start, goal[:-1], limit - 1) + 1,
        minimum_mewtations(start[:-1], goal[:-1], limit - \
            (start[-1] != goal[-1])) + (start[-1] != goal[-1]))
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        the_time = []
        for j in range(0, len(times_per_player[0]) - 1):
            the_time.append(times_per_player[i][j + 1] - \
...
def fastest_words(match):
    ...
    times, words = get_times(match), get_words(match)
    fastest_words = [[] for _ in player_indices]
    for i in word_indices:
        the_fast, index = 0x3f3f3f3f, 0
        for j in player_indices:
            if times[j][i] < the_fast:
                the_fast = times[j][i]
                index = j
        fastest_words[index].append(words[i])
