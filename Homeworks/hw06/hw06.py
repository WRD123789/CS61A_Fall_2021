...
class Coin:
    ...
    def worth(self):
        stamp = Mint.present_year - self.year
        if stamp <= 50:
            return self.cents
        else:
            return self.cents + stamp - 50
...
def store_digits(n):
    ...
    def func(num):
        if num < 10:
            ...
            return Link(num % 10, func(num // 10))
    return func(temp)
...
class VirFib():
    ...
    def next(self):
        if not self.value:
            ...
            new_vir = VirFib(self.value + self.pre_val)
        new_vir.pre_val = self.value
        return new_vir
...
def bst_min(t):
    if t.is_leaf():
        return t.label
    res = min([bst_min(branch) for branch in t.branches])
    return min(res, t.label)

def bst_max(t):
    if t.is_leaf():
        return t.label
    res = max([bst_max(branch) for branch in t.branches])
    return max(res, t.label)
...
def is_bst(t):
    ...
    if br_len > 2:
        ...
        branch = t.branches[0]
        if (bst_max(branch) <= t.label or \
            bst_min(branch) > t.label) and is_bst(branch):
            return True
        else:
            return False
    elif br_len == 2:
        left_br, right_br = t.branches[0], t.branches[1]
        if bst_max(left_br) <= t.label and is_bst(left_br) and \
            bst_min(right_br) > t.label and is_bst(right_br):
            return True
        else:
            return False
