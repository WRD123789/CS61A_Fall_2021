...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        800
        """
        opponent_card.attack = max(0, opponent_card.attack - \
            opponent_card.defense)
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        orig_attack, orig_defense = \
            opponent_card.attack, opponent_card.defense

        for i in range(len(player.deck.cards)):
            player.deck.cards[i].attack += orig_attack
            player.deck.cards[i].defense += orig_defense

        copy_cards = opponent.deck.cards[:]
        for card in copy_cards:
            if card.attack == orig_attack or \
                card.defense == orig_defense:
