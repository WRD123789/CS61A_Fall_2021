...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        min_branch = min(t.branches, key=lambda t: t.label)
        prune_min(min_branch)
...
def address_oneline(text):
    ...
    cardinal_dir = r'[A-Z][a-zA-Z]*\s'  # whitespace is important!
    street = r'(?:[A-Z][a-zA-Z]*\s)*'
    type_abbr = r'\b[A-Z][a-zA-Z]{1,4}\b)'
...
class Player:
    ...
    def debate(self, other):
        cal_pro = max(0.1, self.popularity / (self.popularity + other.popularity))
        cur_pro = random()
        if cur_pro < cal_pro:
            self.popularity += 50
        else:
...
def add_trees(t1, t2):
    ...
    for br1, br2 in zip(t1.branches, t2.branches):
        new_branches.append(add_trees(br1, br2))

    br1_len, br2_len = len(t1.branches), len(t2.branches)
    if br1_len < br2_len:
        new_branches.extend(t2.branches[br1_len:])
    elif br1_len > br2_len:
        new_branches.extend(t1.branches[br2_len:])
