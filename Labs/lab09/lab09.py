def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + in_list for in_list in nested_list]
...
def subseqs(s):
    ...
    if not s:
        ...
        rest_sub = subseqs(s[1:])
        cur_sub = [seq[:] for seq in rest_sub]
        return insert_into_all(s[0], cur_sub) + rest_sub
...
def non_decrease_subseqs(s):
    ...
    """
    """subseq_helper: Find the non-descending subsequence starting with prev
    and pre is not included in the results.
    """
...
class Account:
    ...
    def __repr__(self):
        dep_num, with_num = 0, 0
        for transaction in self.transactions:
            if transaction[0] == 'deposit':
                dep_num += 1
            elif transaction[0] == 'withdraw':
                with_num += 1
        return f'Accountholder: {self.holder}, Deposits: {dep_num}, Withdraws: {with_num}'
...
def insert(link, value, index):
    ...
    while link != Link.empty:
        if index == cur_index:
            ...
            link.first, link.rest = value, link_copy
            return 
        link = link.rest
        cur_index += 1
...
def long_paths(t, n):
    ...
    if t.is_leaf():
        ...
        for branch in t.branches:
            br_paths = long_paths(branch, n - 1)
            if br_paths:
                for br_path in br_paths:
                    br_path.insert(0, t.label)
                    t_paths.append(br_path)
        return t_paths
